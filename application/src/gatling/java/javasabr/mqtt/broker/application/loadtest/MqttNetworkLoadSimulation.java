...
public class MqttNetworkLoadSimulation extends Simulation {
  ...
  private final FeederBuilder<Object> usersFeeder = listFeeder(clientIds)
    ...
    private final ScenarioBuilder scn = scenario("MQTT Load Test")
        .exec(mqtt("Connect").connect())
        .feed(usersFeeder)
        .exec(mqtt("Subscribe").subscribe("loadtest/#{clientId}/echo").qosAtLeastOnce())
    ...
    private String generateRandomTarget() {
        // This is a placeholder. Need a way to select random client IDs.
        // In Gatling, this might be tricky without a feeder.

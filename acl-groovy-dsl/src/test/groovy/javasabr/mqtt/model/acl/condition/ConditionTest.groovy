...
class ConditionTest extends UnitSpecification implements ValueMatchersAware {
  ...
  def "should test condition"(Condition topicFilter, CallId callId, boolean expectedResult) {
    ...
    when:
        result = topicFilter.not().test(callId)
    then:
        result == !expectedResult
    where:
        callId                                         | topicFilter                                               | expectedResult
        new CallId("username", null, null, null, null) | new AnyOfCondition(Array.of(userNameEquals("username")))  | true
        new CallId("username", null, null, null, null) | new AllOfCondition(Array.of(userNameEquals("username")))  | true
        new CallId("username", null, null, null, null) | new AnyOfCondition(Array.of(userNameEquals("username1"))) | false
        new CallId("username", null, null, null, null) | new AllOfCondition(Array.of(userNameEquals("username1"))) | false

...
class PublishCompleteMqttInMessageTest extends BaseMqttInMessageTest {
  ...
  def "should not allow to put reason 2 times"() {
    ...
    then:
        ...
        with(inMessage) {
          ...
          exception().message == "Property:[$MqttMessageProperty.REASON_STRING] is already presented in message:[$MqttMessageType.PUBLISH_COMPLETE]"
        }
  }
  ...
  def "should not allow invalid message flags"() {
    ...
    then:
        ...
        with(inMessage) {
          exception() instanceof MalformedProtocolMqttException
          exception().message == "Unexpected message flags:[0b0101_0101] in message:[$MqttMessageType.PUBLISH_COMPLETE]"
        }
  }
}

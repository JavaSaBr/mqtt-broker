...
public class PublishInPacket extends MqttReadablePacket {
   ...
   * below.
   * <p>
   * 0 - 00 At most once delivery 1 --01 At least once delivery 2 - 10 Exactly once delivery - - 11 Reserved – must not
   * be used
   * <p>
   * If the Server included a Maximum QoS in its CONNACK response to a Client and it receives a PUBLISH packet with a
   * QoS greater than this, then it uses DISCONNECT with Reason Code 0x9B (QoS not supported) as described in section
   * 4.13 Handling errors.
   * <p>
   * A PUBLISH Packet MUST NOT have both QoS bits set to 1 [MQTT-3.3.1-4]. If a Server or Client receives a PUBLISH
   ...
   * to send this PUBLISH packet. If the DUP flag is set to 1, it indicates that this might be re-delivery of an earlier
   ...
   * existing retained message for this topic and store the Application Message [MQTT-3.3.1-5], so that it can be
   * delivered to future subscribers whose subscriptions match its Topic Name. If the Payload contains zero bytes it is
   * processed normally by the Server but any retained message with the same topic name MUST be removed and any future
   * subscribers for the topic will not receive a retained message [MQTT1476 3.3.1-6]. A retained message with a Payload
   * containing zero bytes MUST NOT be stored as a retained message on the Server [MQTT-3.3.1-7].
   * <p>
   * If the RETAIN flag is 0 in a PUBLISH packet sent by a Client to a Server, the Server MUST NOT store the message as
   * a retained message and MUST NOT remove or replace any existing retained message [MQTT-3.3.1-8].
   * <p>
   * If the Server included Retain Available in its CONNACK response to a Client with its value set to 0 and it receives
   * a PUBLISH packet with the RETAIN flag is set to 1, then it uses the DISCONNECT Reason Code of 0x9A (Retain not
   ...
   * set to 1. Which retained messages are sent is controlled by the Retain Handling Subscription Option. At the time of
   * the Subscription: • If Retain Handling is set to 0 the Server MUST send the retained messages matching the Topic
   * Filter of the subscription to the Client [MQTT-3.3.1-9]. • If Retain Handling is set to 1 then if the subscription
   * did not already exist, the Server MUST send all retained message matching the Topic Filter of the subscription to
   * the Client, and if the subscription did exist the Server MUST NOT send the retained messages. [MQTT-3.3.1-10]. • If
   * Retain Handling is set to 2, the Server MUST NOT send the retained messages
   * <p>
   * If the Server receives a PUBLISH packet with the RETAIN flag set to 1, and QoS 0 it SHOULD store the new QoS 0
   * message as the new retained message for that topic, but MAY choose to discard it at any time. If this happens there
   ...
   * controlled by the Retain As Published subscription option. Refer to section 3.8.3.1 for a definition of the
   * Subscription Options. • If the value of Retain As Published subscription option is set to 0, the Server MUST set
   * the RETAIN flag to 0 when forwarding an Application Message regardless of how the RETAIN flag was set in the
   ...
   * String as defined in section 1.5.4 [MQTT-3.3.2-1].
   * <p>
   * Standards Track Work Product Copyright © OASIS Open 2019. All Rights Reserved. Page 56 of 137 The Topic Name in the
   * PUBLISH packet MUST NOT contain wildcard characters [MQTT-3.3.2-2].
   * <p>
   * The Topic Name in a PUBLISH packet sent by a Server to a subscribing Client MUST match the Subscription’s Topic
   * Filter according to the matching process defined in section 4.7 [MQTT-3.3.2-3]. However, as the Server is permitted
   * to map the Topic Name to another name, it might not be the same as the Topic Name in the original PUBLISH packet.
   * <p>

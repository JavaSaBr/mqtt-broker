...
class NativeImageVerificationTest extends Specification {
  ...
  private static final String BINARY_NAME = "native-image"
  private static final String NETWORK_READY_MARKER = "Started external MQTT network by address"
  private static final String APP_TEST_SOURCES = "../application/src/test/groovy/javasabr/mqtt/broker/application"
  ...
  def setupSpec() {
    ...
    File testResourcesDir = new File("../application/src/test/resources/").canonicalFile
    URL[] urls = [testResourcesDir.toURI().toURL()]
    ClassLoader customClassLoader = new URLClassLoader(urls, getClass().getClassLoader())
    Thread.currentThread().setContextClassLoader(customClassLoader)
    testRunner = new EmbeddedSpecRunner(customClassLoader)
  }
  ...
  def "#test should pass without failures"(String test) {
    given:
        def testSource = new File("${APP_TEST_SOURCES}/${test}.groovy")
            .text
            .replace(
  ...
  private static String[] buildBrokerStartupCommand() {
    ...
    def applicationPropertiesPath = new File("build/native/nativeCompile/application.properties").absolutePath

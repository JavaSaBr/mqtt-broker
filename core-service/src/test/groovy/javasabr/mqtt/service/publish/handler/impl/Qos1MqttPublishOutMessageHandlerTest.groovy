...
class Qos1MqttPublishOutMessageHandlerTest extends QosMqttPublishOutMessageHandlerTest {
  ...
  def "should correctly handle unexpected message as response for publish"() {
    given:
        def publishOutHandler = new Qos1MqttPublishOutMessageHandler(
            defaultSubscriptionService,
            defaultMessageOutFactoryService)
    ...
    when: 'send unexpected publish receive'
  ...
  def "should correctly handle publish ack when unexpected stored trackable meta about the publish"() {
    given:
        def publishOutHandler = new Qos1MqttPublishOutMessageHandler(
            defaultSubscriptionService,
            defaultMessageOutFactoryService)
